...
export const upsertUser = mutation({
  args: {
    ...
    role: v.optional(v.string()), // Must be 'super_admin', 'admin', or 'staff'
    branchIds: v.optional(v.array(v.id('branches'))), // Assigned branches
  ...
  handler: async (ctx: MutationCtx, args: {
    ...
    if (existingUser) {
      ...
      const roleToSet = args.role && validRoles.includes(args.role) ? args.role : existingUser.role
      ...
      // Create new user - default to 'staff' if no role specified
      // Validate role if provided
      const validRoles = ['super_admin', 'admin', 'staff']
      const roleToSet = args.role && validRoles.includes(args.role) ? args.role : 'staff'
