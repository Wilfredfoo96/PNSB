/**
 * POST /api/autocount/delivery-orders-v2/[docKey]/post
 * 
 * Posts a draft delivery order
...
export async function POST(
  ...
  try {
    ...
    const response = await apiClient.postDeliveryOrder(docKey)

    if (!response.success) {
      return NextResponse.json(
        {
          error: response.error || 'Failed to post delivery order',
    ...
    return NextResponse.json({
      success: true,
      message: 'Delivery order posted successfully',
    })
  } catch (error) {
    console.error('Error posting delivery order:', error)
    ...
    return NextResponse.json(
      {
        error: 'Failed to post delivery order',
